# coding: utf-8

"""
    User Profile Details

    Configure your UKG Pro Configuration Codes through UKG Pro APIs. Status: R1 deployment

    The version of the OpenAPI document: v1
    Generated by: https://konfigthis.com
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING
from pydantic import BaseModel, Field, RootModel, ConfigDict


class EmployeeProfiles(BaseModel):
    description: typing.Optional[str] = Field(None, alias='description')

    profile_value: typing.Optional[str] = Field(None, alias='profileValue')

    profile_type: typing.Optional[str] = Field(None, alias='profileType')

    product: typing.Optional[str] = Field(None, alias='product')

    module: typing.Optional[str] = Field(None, alias='module')

    company_id: typing.Optional[str] = Field(None, alias='companyId')

    employee_id: typing.Optional[str] = Field(None, alias='employeeId')

    last_modified_date: typing.Optional[datetime] = Field(None, alias='lastModifiedDate')

    is_cleared: typing.Optional[bool] = Field(None, alias='isCleared')

    is_date_effective: typing.Optional[bool] = Field(None, alias='isDateEffective')

    effective_date: typing.Optional[datetime] = Field(None, alias='effectiveDate')

    record_id: typing.Optional[typing.Union[int, float]] = Field(None, alias='recordId')

    change_record_i_d: typing.Optional[typing.Union[int, float]] = Field(None, alias='changeRecordID')

    is_primary: typing.Optional[bool] = Field(None, alias='isPrimary')

    is_primary_effective_date: typing.Optional[datetime] = Field(None, alias='isPrimaryEffectiveDate')

    original_hire_date: typing.Optional[datetime] = Field(None, alias='originalHireDate')

    model_config = ConfigDict(
        protected_namespaces=(),
        arbitrary_types_allowed=True
    )
